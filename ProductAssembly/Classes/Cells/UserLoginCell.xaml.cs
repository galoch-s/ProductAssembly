...
{
	public partial class UserLoginCell : FastCell
	{
		...
		{
			entity = BindingContext as UserContainer;
			if (entity != null) {
		...
		async void OnAlertClicked(object sender, UIButtonEventArgs e)
		{
			...
			RequestForServer dataForServer = new RequestForServer {
				TypeRequest = (int)TypeDataRequest.UserContainerEditPass,
				DataForUrlRequestList = new List<DataForUrlRequest>() { new DataForUrlRequest {
						Param = "admin_password",
						Value = pass
					} },
				PathUrl = string.Format(Api.ApiUserPassword, entity.Id),
			...
			//	PathApi = string.Format(Api.ApiUserPassword, entity.Id),
		...
		//	entity.AdminUserName = result.AdminUserName;

		//	Device.BeginInvokeOnMainThread(() => {
		//		lblAdminUserName.Text = entity.AdminUserName;
		//		frameContainer.OutlineColor = (Color)App.Current.Resources["appGrayLine"];
		//	});
		//}
